package com.apria.svcs.aesservice.validator;

import com.apria.svcs.dataobject.schema.*;
import com.apria.svcs.dataobject.schema.GetDiagnosisXrefRequestDocument.GetDiagnosisXrefRequest;
import com.apria.svcs.dataobject.schema.impl.GetDiagnosisXrefRequestDocumentImpl;
import org.springframework.validation.Errors;
import org.springframework.validation.Validator;

public class DiagnosisXrefServiceValidator implements Validator {

	    /**
	    * This VALIDATOR validates Diagnosis instances, and any subclasses of Diagnosis too
	    */
	   public boolean supports(Class clazz) {
	      return GetDiagnosisXrefRequestDocumentImpl.class.equals(clazz);
	   }

	   public void validate(Object target, Errors errors) {
		  
		  GetDiagnosisXrefRequestDocument requestDoc = (GetDiagnosisXrefRequestDocument) target;
		  
		  /** get the request object */
		  GetDiagnosisXrefRequest diagReq = requestDoc.getGetDiagnosisXrefRequest();
		  
		  /** get the diagnosis DO */
		  DiagnosisXrefDO [] diagDOReq = diagReq.getDiagnosisXrefDOArray();
		  
		  /** Request cannot be more than 30 */
		  if( diagDOReq == null ){
			  errors.reject(ValidatorConstant.ERR_CDE_REQ_REQUIRED,
		  				new Object[]{ValidatorConstant.REQ_ATTR_REQUIRED},
		  				ValidatorConstant.FIELD_DEFAULT_REQUIRED);
		  }
		  if( diagDOReq != null && diagDOReq.length > 30 ){
			 errors.reject(ValidatorConstant.ERR_CDE_MAX_REQUEST);
		  }
		  
		  for( DiagnosisXrefDO diagDO : diagDOReq ){
			  
				  if( diagDO == null  ){
					  errors.reject(ValidatorConstant.ERR_CDE_REQ_REQUIRED,
					  				new Object[]{ValidatorConstant.REQ_ATTR_REQUIRED},
					  				ValidatorConstant.FIELD_DEFAULT_REQUIRED);
				  }else{			  
					  if( diagDO.getDiagCode() == null ){
							errors.reject(ValidatorConstant.ERR_CDE_FIELD_REQUIRED,
								 		new Object[]{ValidatorConstant.FIELD_DIAG_CODE},
			    		  				ValidatorConstant.FIELD_DEFAULT_REQUIRED);     
					  }
				  }
		  }
	}
}
